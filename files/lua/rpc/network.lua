...
function M.dhcp_leases()
    ...
    if not fs.access(leasefile) then
        return rpc.result_response('network',  { leases = leases })
    end
...
function M.get_networks()
    return rpc.result_response('network',  { networks = get_networks() })
end

function M.get_wan_networks()
    return rpc.result_response('network',  { networks = get_networks_by_route('0.0.0.0', 0) })
end

function M.get_wan6_networks()
    return rpc.result_response('network',  { networks = get_networks_by_route('::', 0) })
end
